...
@interface IGUseViewController ()<UITableViewDelegate,UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UILabel *totalLabel;
@property (weak, nonatomic) IBOutlet UILabel *timeLabel;
@property (weak, nonatomic) IBOutlet UIButton *startBtn;
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, strong) NSMutableArray<IGOrderModel*> * dataArr;
...
@end
@implementation IGUseViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    if (self.isState) {
        ...
        IGRoomModel * roomModel = [self.model.roomArr firstObject];;
        ...
        for (IGOrderModel*orderModel in self.dataArr) {
...
-(void)orderClick{
    if (self.isSuspended) {
        IGOrderViewController * orderVC = [[IGOrderViewController alloc]init];
        orderVC.itemBlock = ^(IGOrderModel * _Nonnull orderModel) {
...
- (IBAction)timeSelectClick:(UIButton *)sender {
    ...
    if (self.time_count==0) {
        ...
        IGRoomModel * roomModel = [[IGRoomModel alloc]init];
...
- (IBAction)settlementRoomClick:(UIButton *)sender {
    if (self.time_count==0) {
        ...
        [array replaceObjectAtIndex:self.index_count withObject:self.model];
        [JSUserInfo shareManager].eventArr = array;
        IGSettlementViewController * orderVC = [[IGSettlementViewController alloc]init];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    IGOrderModel * orderModel = self.dataArr[indexPath.section];
    IGMenuTableViewCell * cell = [IGMenuTableViewCell cellWithTableView:tableView];
...
-(NSMutableArray<IGOrderModel *> *)dataArr{
    if (!_dataArr) {
